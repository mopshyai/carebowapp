        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              if (!result.success)
                throw new Error(result.error || 'CareBow could not revoke access.');
              await loadGrants(profileId);
            } catch (error) {
              Alert.alert(
      ...
      >
        <View style={styles.infoCard}>
          ...
          <Text style={styles.infoText}>
            READ ONLY can view profile health data. FULL can also update permitted care data and
            make bookings. Only the profile owner can grant or revoke access.
          </Text>
        </View>
        ...
        {profiles.length > 0 ? (
              ...
              <View style={styles.accessRow}>
                {(['READ_ONLY', 'FULL'] as AccessLevel[]).map((level) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.accessOption,
                      accessLevel === level && styles.accessOptionSelected,
                    ]}
            ...
            {loading ? (
              ...
              <View style={styles.emptyCard}>
                ...
                <Text style={styles.emptyText}>
                  Only the profile owner currently has CareBow access.
                </Text>
              </View>
            ) : (
              grants.map((grant) => (
                <View key={grant.userId} style={styles.grantCard}>
                  ...
                  <View style={styles.grantInfo}>
                    <Text style={styles.grantName}>
                      {grant.name || grant.email || 'CareBow user'}
                    </Text>
          ...
          <View style={styles.emptyCard}>
            ...
            <Text style={styles.emptyText}>
              Create a family health profile before sharing access.
            </Text>
          </View>
        )}
      </ScrollView>
