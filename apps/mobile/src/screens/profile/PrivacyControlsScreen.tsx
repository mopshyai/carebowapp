...
export default function PrivacyControlsScreen() {
  ...
  const beginDeleteAccount = async () => {
    ...
    try {
      ...
      Alert.alert(
        ...
        error instanceof Error ? error.message : 'Your account was not changed. Please try again.'
      );
    } finally {
      setDeleting(false);
    }
  ...
  const confirmDeleteAccount = async () => {
    const ready = deleteMode === 'password' ? Boolean(deletePassword) : /^\d{6}$/.test(deleteCode);
    if (!ready || deleting) return;

    setDeleting(true);
    try {
      const result = await authApi.deleteAccount(
        deleteMode === 'password' ? { password: deletePassword } : { confirmationCode: deleteCode }
      );
      if (!result.success) {
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity
                style={[
                  styles.modalButton,
                  styles.deleteButton,
                  (!deleteInputReady || deleting) && styles.disabled,
                ]}
